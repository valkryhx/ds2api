...
func (c *Client) UploadFile(ctx context.Context, a *auth.RequestAuth, req UploadFileRequest, maxAttempts int) (*UploadFileResult, error) {
	...
	for attempts < maxAttempts {
		...
		resp, err := c.doUpload(ctx, c.regular, c.fallback, DeepSeekUploadFileURL, headers, body)
		if err != nil {
		...
		code := intFrom(parsed["code"])
		msg, _ := parsed["msg"].(string)
		bizCode, bizMsg := deepSeekBizStatus(parsed)
		...
		if a.UseConfigToken {
			...
			if !refreshed && isTokenInvalid(resp.StatusCode, firstNonZeroInt(code, bizCode), statusMsg) {
...
func deepSeekBizStatus(resp map[string]any) (int, string) {
	data, _ := resp["data"].(map[string]any)
	bizCode := intFrom(data["biz_code"])
	bizMsg, _ := data["biz_msg"].(string)
	if bizData, _ := data["biz_data"].(map[string]any); bizCode == 0 {
		bizCode = intFrom(bizData["biz_code"])
		if strings.TrimSpace(bizMsg) == "" {
			bizMsg, _ = bizData["biz_msg"].(string)
		}
	}
